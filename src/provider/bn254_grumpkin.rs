...
use ::grumpkin::bn256::{self, Point as Bn256, Affine as Bn256Affine, Compressed as Bn256Compressed};
use ::grumpkin::grumpkin::{self, Point as GrumpkinPoint, Affine as GrumpkinAffine, Compressed as GrumpkinCompressed};
...
mod tests {
  ...
  fn from_label_serial(label: &'static [u8], n: usize) -> Vec<bn256::Affine> {
    let mut shake = Shake256::default();
    shake.input(label);
    let mut reader = shake.xof_result();
    let mut ck = Vec::new();
